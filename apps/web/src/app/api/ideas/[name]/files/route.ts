/**
 * GET /api/ideas/[name]/files                    — list the cell's root directory.
 * GET /api/ideas/[name]/files?path=src           — list a subdirectory (lazy tree).
 * GET /api/ideas/[name]/files?read=src/server.js — read a file by full relative path.
 */
...
export async function GET(request: Request, context: RouteContext): Promise<Response> {
  try {
    ...
    if (readPath !== null && readPath.length > 0) {
      const result = await getOnCell().readFile(idea.cellId, readPath)
    ...
    const result =
      dirPath.length > 0
        ? await getOnCell().listFiles(idea.cellId, dirPath)
        : await getOnCell().listFiles(idea.cellId)
