...
export function IdeaActions({ name }: IdeaActionsProps) {
  ...
  async function handleSnapshot() {
    await run(async () => {
      const data = await apiFetch<{ snapshot: { key: string } }>(
        `/api/ideas/${encodeURIComponent(name)}/snapshot`,
        { method: 'POST' }
      )
  ...
  async function handleFork() {
    ...
    await run(async () => {
      await apiFetch(`/api/ideas/${encodeURIComponent(name)}/fork`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ name: target })
      })
      router.push(`/ideas/${encodeURIComponent(target)}`)
      router.refresh()
    })
  }
    ...
    <div className="flex flex-col gap-3">
      ...
      {dialog === 'fork' && (
        <div className="panel flex flex-col gap-3 p-4">
          <p className="text-sm text-muted">
            Fork copies everything — the app, its files, and its state — into a new idea you can
            take in a different direction.
          </p>
          <input
      ...
      {dialog === 'delete' && (
        <div className="panel flex flex-col gap-3 border-bad/40 p-4">
          <p className="text-sm text-muted">
            Delete <span className="font-mono text-fg">{name}</span> from your ideas
            {keepRemote ? '.' : ' and delete its workspace cell.'} This cannot be undone.
          </p>
